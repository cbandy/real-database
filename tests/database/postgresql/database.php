...
class Database_PostgreSQL_Database_Test extends PHPUnit_Framework_TestCase
{
	...
	public function setUp()
	{
		...
		$db->execute_command('CREATE TEMPORARY TABLE '.$this->_table.' ("id" bigserial PRIMARY KEY, "value" integer)');
		...
		$db->execute_command('INSERT INTO '.$this->_table.' ("value") VALUES (65)');
		$db->execute_command('INSERT INTO '.$this->_table.' ("value") VALUES (65)');
	}
	...
	public function test_copy_from()
	{
		...
		$db->copy_from('temp_test_table', array("8\t\\N", "9\t75"));
		...
		$this->assertEquals(array(
			...
			array('id' => 9, 'value' => 75),
		), $db->execute_query('SELECT * FROM '.$this->_table.' ORDER BY "id"')->as_array());
	}
	...
	public function test_copy_to()
	{
		...
		$db->execute_command('INSERT INTO '.$this->_table.' ("value") VALUES (NULL)');
	...
	public function test_execute_command_query()
	{
		...
		$this->assertSame(5, $db->execute_command('SELECT * FROM '.$this->_table), 'Number of returned rows');
	}
	...
	public function test_execute_compound_command()
	{
		...
		$this->assertSame(2, $db->execute_command('DELETE FROM '.$this->_table.' WHERE "id" = 3; DELETE FROM '.$this->_table.' WHERE "id" = 5'), 'Total number of rows');

		try
		{
			// Connection should have no pending results
			$db->execute_query('SELECT * FROM '.$this->_table);
		}
		catch (Exception $e)
		{
	...
	public function test_execute_copy()
	{
		...
		$this->assertSame(0, $db->execute_command('COPY '.$this->_table.' TO STDOUT'));

		$this->assertNull($db->execute_query('COPY '.$this->_table.' TO STDOUT'));
	}

	public function test_execute_prepared_command()
	{
		$db = $this->sharedFixture;
		$name = $db->prepare(NULL, 'UPDATE '.$this->_table.' SET "value" = 20 WHERE "value" = 65');

		$this->assertSame(2, $db->execute_prepared_command($name));

		$name = $db->prepare(NULL, 'UPDATE '.$this->_table.' SET "value" = $1 WHERE "value" = $2');
	...
	{
		$db = $this->sharedFixture;
		$name = $db->prepare(NULL, 'SELECT * FROM '.$this->_table.' WHERE "value" = $1');
		...
		$name = $db->prepare(NULL, 'SELECT * FROM '.$this->_table);
	...
	public function test_prepare()
	{
		...
		$this->assertSame('asdf', $db->prepare('asdf', 'SELECT * FROM '.$this->_table));
	}

	public function test_prepare_command()
	{
		$db = $this->sharedFixture;
		$query = $db->prepare_command('DELETE FROM '.$this->_table);
		...
		$query = $db->prepare_command('DELETE FROM ? WHERE :cond', array(new Database_Table('temp_test_table'), ':cond' => new Database_Conditions(new Database_Column('value'), '=', 60)));
	...
	{
		$db = $this->sharedFixture;
		$query = $db->prepare_query('SELECT * FROM '.$this->_table);
		...
		$query = $db->prepare_query('SELECT * FROM ? WHERE :cond', array(new Database_Table('temp_test_table'), ':cond' => new Database_Conditions(new Database_Column('value'), '=', 60)));
	...
	public function test_savepoint_transactions()
	{
		...
		$db->begin();
		$db->execute_command('DELETE FROM '.$this->_table.' WHERE "value" = 65');

		$this->assertSame(3, $db->execute_query($select)->count(), 'Deleted 65');

		$this->assertNull($db->savepoint('test_savepoint'));

		$db->execute_command('DELETE FROM '.$this->_table.' WHERE "value" = 55');
